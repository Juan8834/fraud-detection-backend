...
async function main() {
  ...
  const employees: Prisma.EmployeeUncheckedCreateInput[] = [];
  ...
  const customers: Prisma.CustomerUncheckedCreateInput[] = [];
  ...
  // Transactions
  const transactionTypes = [TransactionType.SALE, TransactionType.REFUND, TransactionType.FRAUD];
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const totalAmount = selectedItems.reduce(
      (sum, item) => sum + item.price! * getRandomInt(1, 5), // random quantity multiplier
      0
    );

    const fraudScenario =
      type === TransactionType.FRAUD
    ...
    await prisma.transaction.create({
      data: {
        ...
        totalAmount,
        riskScore: type === TransactionType.FRAUD ? getRandomInt(60, 100) : getRandomInt(0, 40),
