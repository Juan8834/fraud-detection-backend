...
router.get("/", async (req, res) => {
    try {
        ...
        const reformatted = transactions.map((tx) => {
            ...
            const totalAmount = tx.totalAmount ?? items.reduce((sum, item) => sum + item.total, 0);
            // Determine if this transaction should be flagged as fraud
            const isFraud = (tx.riskScore ?? 0) >= 75 ||
                !!tx.fraudType ||
...
router.patch("/:id/case", async (req, res) => {
    ...
    try {
        ...
        const totalAmount = updatedTx.totalAmount ?? items.reduce((sum, item) => sum + item.total, 0);
        const isFraud = (updatedTx.riskScore ?? 0) >= 75 ||
            !!updatedTx.fraudType ||
